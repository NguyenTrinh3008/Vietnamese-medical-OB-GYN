#!/usr/bin/env python3
"""
Fine-tune mDeBERTa-v3 for Vietnamese Medical NLI / Hallucination Detection

Based on research paper recommendations:
1. mDeBERTa-v3 with disentangled attention (better for Vietnamese word order)
2. Start from XNLI warm-up checkpoint (transfer learning from English NLI)
3. Layer-wise Learning Rate Decay (LLRD) - preserve lower layer features
4. Weighted Loss for imbalanced classes
5. Freeze embeddings strategy to prevent overfitting
6. Combine ViANLI (adversarial) + Medical examples

Reference: "Kiến trúc và Chiến lược Tinh chỉnh mDeBERTa-v3 cho Hệ thống 
Kiểm tra Ảo giác và Phê bình (Critic) trong RAG Tiếng Việt"
"""
...
# ============================================================
# Configuration - Following Research Paper
# ============================================================

# Use XNLI warm-up checkpoint (key insight from paper!)
MODEL_NAME = "MoritzLaurer/mDeBERTa-v3-base-xnli-multilingual-nli-2mil7"
# Alternative: "microsoft/mdeberta-v3-base"

OUTPUT_DIR = "./mdeberta_v3_medical_nli"
NUM_LABELS = 3

# Training hyperparameters (from paper)
BATCH_SIZE = 8  # Smaller for better gradient updates
GRADIENT_ACCUMULATION = 4  # Effective batch size = 32
BASE_LEARNING_RATE = 5e-5  # For classifier head
LAYER_DECAY = 0.9  # LLRD decay factor
NUM_EPOCHS = 5
MAX_LENGTH = 256
WARMUP_RATIO = 0.1

# Label mapping (same as paper)
LABEL2ID = {"entailment": 0, "neutral": 1, "contradiction": 2}
ID2LABEL = {0: "entailment", 1: "neutral", 2: "contradiction"}

# Class weights for imbalanced data (prioritize catching hallucinations)
# Paper: [1.0, 1.5, 2.0] for [entailment, neutral, contradiction]
CLASS_WEIGHTS = torch.tensor([1.0, 1.5, 2.0])

print("=" * 90)
print("mDeBERTa-v3 MEDICAL NLI FINE-TUNING (Research Paper Implementation)")
print("=" * 90)

# ============================================================
# Step 1: Create Medical Examples (Data Augmentation)
# ============================================================
...
print(f"   Created {len(medical_examples)} unique medical examples")
print(f"   After repeat: {len(repeated_medical)} examples")

# ============================================================
# Step 2: Load ViANLI Dataset
# ============================================================
print("\n📥 Step 2: Loading ViANLI dataset (adversarial NLI)...")
...
# ============================================================
print(f"\n🔧 Step 3: Loading mDeBERTa-v3 model...")
...
# Verify tokenizer with Vietnamese
test_text = "Thuốc không được chỉ định cho trẻ em"
tokens = tokenizer.tokenize(test_text)
print(f"   Tokenizer test: '{test_text}' -> {tokens}")
...
# ============================================================
# Step 5: Setup LLRD (Layer-wise Learning Rate Decay)
# ============================================================
print("\n⚙️ Step 5: Setting up Layer-wise Learning Rate Decay (LLRD)...")
...
def get_optimizer_grouped_parameters(model, base_lr, layer_decay, weight_decay=0.01):
    ...
    print(f"      Embeddings LR: {lr_embed:.2e}")
    ...
    for layer_i in range(num_layers):
        ...
        if layer_i == 0 or layer_i == num_layers - 1:
            print(f"      Layer {layer_i} LR: {lr_layer:.2e}")
    ...
    return [l for l in layers if len(l["params"]) > 0]

# ============================================================
# Step 6: Custom Trainer with Weighted Loss
# ============================================================
print("\n⚙️ Step 6: Setting up Weighted Loss Trainer...")
...
# Step 7: Training
# ============================================================
print(f"\n🚀 Step 7: Starting training ({NUM_EPOCHS} epochs)...")
print(f"   Batch size: {BATCH_SIZE} x {GRADIENT_ACCUMULATION} = {BATCH_SIZE * GRADIENT_ACCUMULATION}")
print(f"   Base LR: {BASE_LEARNING_RATE}, Layer decay: {LAYER_DECAY}")
...
training_args = TrainingArguments(
    ...
    logging_steps=50,
    bf16=torch.cuda.is_available(),  # Use bf16 for Ampere GPUs (paper recommends)
    report_to="none",
)
...
print(f"             Pred_E  Pred_N  Pred_C")
print(f"   True_E:   {cm[0][0]:5d}   {cm[0][1]:5d}   {cm[0][2]:5d}")
print(f"   True_N:   {cm[1][0]:5d}   {cm[1][1]:5d}   {cm[1][2]:5d}")
print(f"   True_C:   {cm[2][0]:5d}   {cm[2][1]:5d}   {cm[2][2]:5d}")
